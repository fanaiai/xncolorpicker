...
;(function (undefined) {
     ...
     */
    var colorFormat = function colorFormat(options) {
        return new _colorFormat(options);
    };
    var _colorFormat = function _colorFormat(options) {
        var result,
            color = options && options.color && options.color.replace(/\s/g, "").toLowerCase() || "#f00",
            // color ：默认值为 "#f00"
        format = options && options.format && options.format.replace(/\s/g, "").toLowerCase() || "rgb",
            // format ：默认值为 "rgb"
        rgbType = format.indexOf("rgba") == -1 ? 0 : 1,
            // rgbType 0表示rgb，1表示rgba
    ...
    _colorFormat.prototype = {
        constructor: this,
        defineColor: [{ name: "red", hex: "#f00" }, { name: "orange", hex: "#ffa500" }, { name: "yellow", hex: "#ff0" }, { name: "green", hex: "#0f0" }, { name: "cyan", hex: "#0ff" }, { name: "blue", hex: "#00f" }, { name: "violet", hex: "#ee82ee" }, { name: "black", hex: "#000" }, { name: "white", hex: "#fff" }],
        getRgb: function getRgb(rgb, type) {
            /**
            ...
            return {
                ...
                b: blue,
                o: opacity,
                complete: type ? "rgba(" + [red, green, blue, opacity].join(",") + ")" : "rgb(" + [red, green, blue].join(",") + ")"
            };
        },
        getHsl: function getHsl(hsl, type) {
            /**
        ...
        rgbToHex: function rgbToHex(rgb) {
            ...
            return {
                ...
                b: blue,
                o: opacity,
                complete: simpleType ? "#" + red[0] + green[0] + blue[0] + (rgb.o == 1 ? "" : opacity) : "#" + red + green + blue + (rgb.o == 1 ? "" : opacity)
            };
        },
        rgbToHsl: function rgbToHsl(rgb, type) {
            /**
        ...
        hexToRgb: function hexToRgb(hex, type) {
            /**
        ...
        hslToRgb: function hslToRgb(hsl, type) {
            /**
